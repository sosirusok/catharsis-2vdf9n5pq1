      ...
      <div className="theme-list shell">
        {themes.map((theme) => (
          <article className="theme-card" key={theme.id}>
            <div className={`theme-art ${theme.artKey}`} style={theme.imageKey ? { backgroundImage: `url(/api/public/theme-image/${theme.imageKey})` } : undefined} role="img" aria-label={`${theme.name} 테마 이미지`} />
